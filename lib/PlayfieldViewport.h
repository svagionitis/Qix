...
public:
    ...
    constexpr PlayfieldViewport(
        float fieldX, float fieldY, float fieldW, float fieldH,
        std::int32_t gridW = 80, std::int32_t gridH = 60) noexcept
        : m_fieldX {fieldX}
        , m_fieldY {fieldY}
        , m_fieldW {fieldW}
        , m_fieldH {fieldH}
    ...
    {
        return ViewportRect {
            m_fieldX + static_cast<float>(gx) * m_cellW,
            m_fieldY + static_cast<float>(gy) * m_cellH,
            m_cellW + overlap,
            m_cellH + overlap
        };
    }
        ...
        std::int32_t gx, std::int32_t gy, int texW, int texH) const noexcept
    {
        ...
        return ViewportPixelRect {
            static_cast<int>((static_cast<double>(gx) / m_gridW) * tw),
    ...
    [[nodiscard]] static constexpr int centerX(
        int elementWidth, int containerWidth, int minMargin = 10) noexcept
    {
    ...
    [[nodiscard]] static constexpr bool isBlinkOn(
        std::uint32_t ticksMs, std::uint32_t halfPeriodMs = 350U) noexcept
    {
        return ((ticksMs / (halfPeriodMs > 0U ? halfPeriodMs : 1U)) % 2U) == 0U;
    }

    [[nodiscard]] constexpr float cellWidth() const noexcept { return m_cellW; }
    [[nodiscard]] constexpr float cellHeight() const noexcept { return m_cellH; }
    [[nodiscard]] constexpr std::int32_t gridWidth() const noexcept { return m_gridW; }
    [[nodiscard]] constexpr std::int32_t gridHeight() const noexcept { return m_gridH; }
